...
class MemotionMixerMultiLoss(AbstractTrainTestModule):
    def __init__(self, model_cfg: DictConfig, optimizer_cfg: DictConfig, **kwargs):
        self.num_classes = model_cfg.modalities.classification.get(
            'num_classes', 3)
        super(MemotionMixerMultiLoss, self).__init__(
    ...
    # TODO Rename this here and in `on_validation_epoch_end`
    ...
    def setup_scores(self) -> List[torch.nn.Module]:
        train_scores = dict(f1m=F1Score(task="multiclass",
                            num_classes=self.num_classes, average='macro'))
        val_scores = dict(f1m=F1Score(task="multiclass",
                          num_classes=self.num_classes, average='macro'))
        test_scores = dict(f1m=F1Score(task="multiclass",
                           num_classes=self.num_classes, average='macro'))
